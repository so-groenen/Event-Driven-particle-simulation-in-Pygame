...
In contrast, in "event driven" collisions, a priority queue of all particle collisions is created BEFORE the simulation launch:
The top priority collision event is poped from the queue and as soon as the system time == collision time, the relevant particle-pair collision is handled, then futur collisions involving these particles are computed, and pushed back into the queue.
